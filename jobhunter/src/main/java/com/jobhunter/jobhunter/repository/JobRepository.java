...
public interface JobRepository extends JpaRepository<Job, Long> {
    ...
    // Scheduled Task: Delete expired tokens (use PasswordResetTokenRepository instead).
    // Admin reopen: Just use findById and then update status + expiredAt
    // Dashboard stats
    long countByStatusJob(AppEnums.JobStatus statusJob);

    // Trending: Top job with the most applications in the last 7 days.
    ...
    /**
     * Get job list with candidateCount + pendingCount.
     * Sort: job have new application to top list (MAX appliedAt DESC NULLS LAST).
     */
    @Query("""
    ...
    // Count applied of job (for detail)
